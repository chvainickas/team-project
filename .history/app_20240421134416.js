...
app.post("/saveChatHistory", function (req, res) {
  ...
  const dir = path.join(
    ...
    "user_data",
    username,
    chatHistory[0].subject
  );
  // Read the existing chat history
  let filename = ""; // Declare the filename variable
  ...
  fs.readFile(path.join(dir, filename), "utf8", (err, data) => {
    ...
    // Update the filename based on the first question if it's the first chat message
