...
app.post("/saveChatHistory/:subject", function (req, res) {
  ...
  fs.readFile(path.join(dir, `${subject}.json`), "utf8", (err, data) => {
    ...
    // Extract the first question from the chat history
    const firstQuestionMessage = chatHistory.find(message => message.user.type === 'question');

    // If a question was found, use its content as the file name
    // Otherwise, use the subject as the file name
    const filename = firstQuestionMessage
      ? `${firstQuestionMessage.content.replace(/[<>:"/\\|?*]+/g, ' ')}.json`
      : `${subject}.json`;
...
async function getChatFiles(username, subjectFolder) {
  // Decode the subjectFolder parameter
  subjectFolder = decodeURIComponent(subjectFolder);

  const dirPath = path.join(__dirname, "user_data", username, subjectFolder);
  try {
    const files = await fs.promises.readdir(dirPath);
